...
<div id="tv-image-preview-{$tv->id}" class="modx-tv-image-preview">
    {if $tv->value}<img src="{$_config.connectors_url}system/phpthumb.php?w=200&h=100&f=png&src={$tv->value}&source={$source}" alt="" />{/if}
</div>
{if $disabled}
        ...
        ,listeners: {
            'select': {fn:function(data) {
                ...
                if (Ext.isEmpty(data.url)) {
                    ...
                    {/literal}
                    d.update('<img src="{$_config.connectors_url}system/phpthumb.php?w=200&h=100&f=png&src='+data.url+'&wctx={$ctx}&source={$source}" alt="" />');
                    {literal}
                }
            }}
        }
